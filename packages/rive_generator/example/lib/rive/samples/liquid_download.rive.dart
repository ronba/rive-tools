...
class Liquid_download {
  ...
  static Future<Liquid_download> load() async {
    final riveFile = rive.RiveFile.import(await rootBundle.load('samples/liquid_download.riv')); 
    return Liquid_download._(riveFile);
  }

  Artboard? _artboard;
  Artboard get artboard => _artboard ??= Artboard(file.artboardByName('Artboard')!);
...
class Artboard {
  ...
  ArtboardDownloadStateMachine getArtboardDownloadStateMachine([core.OnStateChange? onStateChange]) {
    return ArtboardDownloadStateMachine(this.artboard.stateMachineByName("Download",onChange: onStateChange)!);
  }
}
